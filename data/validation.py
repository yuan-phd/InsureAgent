...
def validate_traces(jsonl_path: str) -> dict:
    ...
    for i, trace in enumerate(traces):
        try:
            ...
            if len(convs) < 4:
                errors.append({"index": i, "error": "fewer than 4 turns"})
                continue

            # First turn must be system
            if convs[0]["role"] != "system":
                errors.append({"index": i, "error": "first turn not system"})
                continue
            ...
            if tool_calls < 2:
                errors.append({"index": i, "error": f"only {tool_calls} tool calls"})
                continue
            ...
            if not has_verdict:
                errors.append({"index": i, "error": "missing Verdict"})
                continue
            ...
            if hallucinated:
                errors.append({"index": i, "error": "hallucinated Observation"})
                continue
